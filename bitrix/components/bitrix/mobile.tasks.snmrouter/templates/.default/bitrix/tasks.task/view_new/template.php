...
$task["SHOW_DISK_FIELD"] = is_array($arResult["AUX_DATA"]) && is_array($arResult["AUX_DATA"]["USER_FIELDS"]) &&
...
function getChecklistFields(&$task, $can)
{
	$actions = '';
	if($can["CHECKLIST_ADD_ITEMS"]) {
		ob_start();?>
	...
	ob_start();
	?>
	<div class="task-mobile-no-data task-mobile-no-data-checklist" id="task-mobile-no-data-checklist" style="<?=!empty($task["SE_CHECKLIST"])?'display:none;':''?>">
		<div class="task-mobile-no-data-inner">
			<div class="task-mobile-no-data-image"></div>
			<div class="task-mobile-no-data-text"><?=GetMessage('TASKS_EMPTY_BLOCK_CHECKLIST')?></div>
		</div>
	</div>
	<?php
	$html = ob_get_clean();
	...
	$result = [
		...
		"value" => "<div id=\"checkList".$task["ID"]."Container\" class=\"mobile-grid-field-tasks-checklist-container\">"
			. $checkList($task["SE_CHECKLIST"], $task["CHECKLIST"])
			. "</div>"
			. $actions
			. $html
	];
...
function getPriorityField($task, $can)
{
	$data = [
		"id" => "data[PRIORITY]",
	...
	{
		ob_start();?>
		<label id="mobile-grid-field-priority-<?=$task["ID"]?>" class="mobile-grid-field-priority mobile-grid-field-priority-<?=$task["PRIORITY"]?>">
			<span><?=GetMessage("MB_TASKS_BASE_SETTINGS_PRIORITY_VALUE")?></span>
		</label>
		<?php
		$html = ob_get_clean();
	...
	array(
		...
		'TABS' => array( // Tabs to show for mobile version it is redundant just to keep compatibility with web version
				...
				"fields" => array(
					array(
						...
						"value" => (
							...
							.($task["NOT_VIEWED"] == 'Y' ? '<div class="mobile-grid-field-expired">'.GetMessage("MB_TASKS_TASK_DETAIL_NOTIFICATION_NEW_BRIEF").'</div>' : null)
			...
			array(
				...
				"title" => '',
				"name" => '',
				"fields" => getChecklistFields($task, $can)
			),
			array(
				"id" => "task_files",
				"title" => '',
				...
				"fields" => array(
					...
					[
						"type" => "custom",
						"id" => "data[UF_TASK_WEBDAV_FILES_EMPTY]",
						"name" => GetMessage("MB_TASKS_TASK_SETTINGS_DISK_UF"),
						"show" => ($task["SHOW_DISK_FIELD"] ? "N" : "Y"),
						"value" => sprintf(<<<HTML
<div id="block_file_empty">
	<div class="task-mobile-no-data task-mobile-no-data-file">
		<div class="task-mobile-no-data-inner">
			<div class="task-mobile-no-data-image"></div>
			<div class="task-mobile-no-data-text">%s</div>
		</div>
	</div>
</div>
HTML
							, GetMessage('TASKS_EMPTY_BLOCK_FILES')
						)
					]
				)
			)
...
BX.ready(function(){
	...
	var checkedPriority = document.querySelector('#bx_data_PRIORITY_1');
	if(checkedPriority != null)
	{
		checkedPriority.nextSibling.innerHTML = BX.message('TASKS_PRIORITY_' + (checkedPriority.checked ? 2 : 0));
	}

	BXMobileApp.addCustomEvent("tasks.task.update", function(data){
		console.log("Event: tasks.task.update", data);
		if(data.hasOwnProperty('favorite'))
		{
			document.querySelector('#favorites<?=$task["ID"]?>').classList.toggle('active');
		}
		if(data.hasOwnProperty('status'))
		{
			const statuses = {
				1: BX.message('TASKS_STATUS_STATE_NEW'),
				2: BX.message('TASKS_STATUS_STATE_PENDING'),
				3: BX.message('TASKS_STATUS_STATE_IN_PROGRESS'),
				4: BX.message('TASKS_STATUS_STATE_SUPPOSEDLY_COMPLETED'),
				5: BX.message('TASKS_STATUS_STATE_COMPLETED'),
				6: BX.message('TASKS_STATUS_STATE_DEFERRED'),
				7: BX.message('TASKS_STATUS_STATE_DECLINED')
			};

			document.querySelector('#bx-task-status-<?=$task["ID"]?>').innerHTML = statuses[Number(data.status)];
		}
	});
	...
	setTimeout(function(){
		...
		{
			// window.BXMobileApp.UI.Page.TextPanel.hide();
	...
	// var tabs = ["commentTab", "filesTab", "checklistTab", "taskTab"];
	var tabId = (new URL(location.href)).searchParams.get("tabId");
	if(tabId === 'commentTab')
		tabId = 'commentTab';
	else
		<?php
			if((int)$task["~STATUS"] === MetaStatus::UNSEEN)
			{
				?>tabId = 'taskTab';<?
			}
	...
	BX.onCustomEvent('onTabSelected', [{tab:{id:tabId}}]);
	BXMobileApp.Events.postToComponent('onTabSelect', [{tab: tabId}], 'tasks.view');

	BX.message({MB_TASKS_ADD_COMMENT_FIRST: '<?=GetMessage('MB_TASKS_ADD_COMMENT_FIRST')?>'});

	BX.addCustomEvent('onMobileTaskViewCheckListAdd', function(data){
		console.log('onMobileTaskViewCheckListAdd', data);
		document.querySelector('#task-mobile-no-data-checklist').style.display = 'none';
	});

	BX.addCustomEvent('onMobileTaskViewCheckListRemove', function(data){
		console.log('onMobileTaskViewCheckListRemove', data);
		if(data.length==0)
		{
			document.querySelector('#task-mobile-no-data-checklist').style.display = 'flex';
		}
	});
});
...
BX.addCustomEvent("onCheckFileBlockVisibility", function(node){
	...
	{
		node.parentNode.appendChild(BX("block_file_empty"));
	}
	if (node.childNodes.length <= 0)
	{
		BX.show(BX("block_file_empty"));
	}
	else
	{
		BX.hide(BX("block_file_empty"));
	}
})
</script>

<div id="empty_block" style="display: none;">
	<div class="task-mobile-no-data">
		<div class="task-mobile-no-data-inner">
			<div class="task-mobile-no-data-image"></div>
			<div class="task-mobile-no-data-text" id="empty_text">text</div>
		</div>
	</div>
</div>