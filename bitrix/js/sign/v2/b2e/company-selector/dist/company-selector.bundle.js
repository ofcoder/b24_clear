...
(function (exports,main_core,main_date,main_loader,main_popup,sign_v2_api,sign_v2_companyEditor,sign_v2_helper,ui_entitySelector,sign_tour,ui_label,ui_alerts) {
	'use strict';
	...
	const allowedSignatureProviders = ['goskey', 'taxcom', 'ses-ru', 'ses-com'];
	...
	class CompanySelector {
	  ...
	  async save(documentId) {
	    ...
	    return babelHelpers.classPrivateFieldLooseBase(this, _api)[_api].modifyB2eCompany(documentId, provider.uid);
	  }
	}
	...
	function _renderProviderInfo2(provider = null) {
	  ...
	  if (code === '') {
	    ...
	    const text = main_core.Tag.render(_t33 || (_t33 = _`<span>${0}</span>`), providerInfo);
	    ...
	    var _providerCodeToProvid, _providerCodeToHelpde;
	    ...
	    const text = main_core.Tag.render(_t34 || (_t34 = _`<span>${0}</span>`), sign_v2_helper.Helpdesk.replaceLink((_providerCodeToProvid = providerCodeToProviderInfoTextMap[code]) != null ? _providerCodeToProvid : '', (_providerCodeToHelpde = providerCodeToHelpdeskCodeMap[code]) != null ? _providerCodeToHelpde : ''));
	    main_core.Dom.append(text, babelHelpers.classPrivateFieldLooseBase(this, _ui)[_ui].provider.info);
	...
	function _getConnectedName2(provider, rqInn) {
	  const providerName = babelHelpers.classPrivateFieldLooseBase(this, _getProviderNameByCode)[_getProviderNameByCode](provider.code);
	  ...
	  if (company && company.registerUrl) {
	    ...
	    BX.SidePanel.Instance.open('sign:stub', {
	      ...
	      contentCallback: () => {
	        ...
	        babelHelpers.classPrivateFieldLooseBase(this, _registerIframe)[_registerIframe] = main_core.Tag.render(_t35 || (_t35 = _`<iframe src="${0}" style="${0}"></iframe>`), company.registerUrl, frameStyles);
	...
	async function _registerVirtualProviderIfNeed2() {
	  ...
	  const {
	    id
	  } = await babelHelpers.classPrivateFieldLooseBase(this, _api)[_api].registerB2eCompany(babelHelpers.classPrivateFieldLooseBase(this, _company)[_company].provider.code, selectedItem.rqInn);
...
}((this.BX.Sign.V2.B2e = this.BX.Sign.V2.B2e || {}),BX,BX.Main,BX,BX.Main,BX.Sign.V2,BX.Sign.V2,BX.Sign.V2,BX.UI.EntitySelector,BX.Sign.Tour,BX.UI,BX.UI));
